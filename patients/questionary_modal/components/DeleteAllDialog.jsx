...
const DeleteAllDialog = observer(() => {
    ...
    const close = () => {
        questionaryModalViewStore.setOpenDeleteAll(false)
    }

    const deleteAndClose = () => {
        setIsloading(true)
        questionaryModalStore.deleteAllFiles(patientStore.getPatientId(), patientStore.getLastName(), patientStore.getFirstName()).then(() => {
            setIsloading(false)
            close()
        })
    }
    ...
    return (
        <Dialog
            open={questionaryModalViewStore.getOpenDeleteAll()}
            onClose={() => close()}
