...
const DeleteOneDialog = observer(() => {
    ...
    const close = () => {
        questionaryModalViewStore.setOpenDeleteOne(false)
    }

    const deleteAndClose = () => {
        setIsloading(true)
        questionaryModalStore.deleteOneFile(patientStore.getPatientId(), patientStore.getLastName(), patientStore.getFirstName()).then(() => {
            setIsloading(false)
            close()
        })
    }
    ...
    return (
        <Dialog
            open={questionaryModalViewStore.getOpenDeleteOne()}
            onClose={() => close()}
